...
SUBROUTINE  partialdos (Emin, Emax, DeltaE, kresolveddos, filpdos)
  ...
  !SUBHAYAN
  ALLOCATE (hyb_pdos(0:ne,nproj,nproj,nspin,nkseff))
  ...
  !write(stdout,*)"What method?"
  IF ( ltetra .AND. tetra_type == 1 .OR. tetra_type == 2 ) THEN
     !
     ! Tetrahedron method
     write(stdout,*)"Tetrahedron method"
     !
  ...
  ELSE
     !
  !OPEN (77,file="HybPdos.dat",form='formatted', status='unknown')
  OPEN (78,file="hybrid_pdos.dat",form='formatted', status='unknown')
  write(78,*)"#PDOS(\alpha,E)=sum_{n,k} \braket{\alpha|\psi_{n,k}}\braket{\psi_{n,k}|\alpha} 1/{\sigma\sqrt{\pi}}exp(-(E-E_n)^2/\sigma^2)"
  write(78,*)"#Hybrid PDOS(\alpha,\beta,E)="
  write(78,*)"#sum_{n,k} \braket{\alpha|\psi_{n,k}} \braket{\psi_{n,k}|\beta} 1/{\sigma\sqrt{\pi}}exp(-(E-E_n)^2/\sigma^2)"
  write(78,*)"#This file prints Hybrid PDOS"
  write(78,*)"# spin orbital1 orbital2 energy hybrid_pdos"
     DO ik = 1,nkstot
        !
        IF (kresolveddos) THEN
        ...
        DO ibnd = 1, nbnd
           ...
           !
              DO nwfc = 1, nproj
                ...
                !SUBHAYAN
                DO jwfc = 1, nproj
                 hyb_pdos(ie,nwfc,jwfc,current_spin,ikeff) = hyb_pdos(ie,nwfc,jwfc,current_spin,ikeff) + &
                      wkeff * delta * ((proj (nwfc, ibnd, ik) * proj (jwfc, ibnd, ik))**0.5)
                ENDDO
              ENDDO
           !
...
!            DO ie = max(ie_mid-ie_delta, 0), min(ie_mid+ie_delta, ne)
!             DO nwfc = 1, nproj
!              write(stdout,*)"PDOS_PRINTING",ie,nwfc,current_spin,ikeff,pdos(ie,nwfc,current_spin,ikeff)
!             ENDDO
!            ENDDO
!           ENDIF
        ENDDO 
     ...
     DO nwfc = 1, nproj
      DO is=1,nspin
       write(77,*)"Printing for nwfc = ",nwfc,"and spin = ",is    
       DO ik=1,nkseff
        DO ie= 0, ne
         etev = Emin + ie * DeltaE
         !write(77,'(f7.3,14e11.3)')etev*rytoev, (hyb_pdos(ie,nwfc,jwfc,is,ik),jwfc=1, nproj)
         DO jwfc=1, nproj
          write(78,'(I1,1x,I6,1x,I6,1x,f7.3,1x,14e11.3)') is, nwfc, jwfc, etev*rytoev,hyb_pdos(ie,nwfc,jwfc,is,ik)
         ENDDO
        ENDDO
       ENDDO
      ENDDO
     ENDDO
     !SUBHAYAN
     write(stdout,*)"start printing proj"
     DO ik=1,nkstot
      DO ibnd = 1, nbnd
       DO nwfc = 1, nproj
        IF (nspin .eq. 1) THEN
         write(stdout,'(A25,1x,I4,1x,I6,1x,I7,1x,F17.10)')"ispin,ik,ibnd,nwfc,proj 1",ik,ibnd,nwfc,proj(nwfc, ibnd, ik)
        ENDIF
        IF (nspin .eq. 2) THEN
      if (ik .le. int(nkstot*1.0/2.0)) write(stdout,'(A25,1x,I4,1x,I6,1x,I7,1x,F17.10)')"ispin,ik,ibnd,nwfc,proj 1",ik,ibnd,nwfc,proj(nwfc, ibnd, ik)
      if (ik .gt. int(nkstot*1.0/2.0)) write(stdout,'(A25,1x,I4,1x,I6,1x,I7,1x,F17.10)')"ispin,ik,ibnd,nwfc,proj 2",(ik-int(nkstot*1.0/2.0)),ibnd,nwfc,proj(nwfc,ibnd,ik)
        ENDIF
       ENDDO
      ENDDO
     ENDDO
     write(stdout,*)"done printing proj"
     !write(stdout,*)"START PRINTING PDOS"
     !DO ik = 
  END IF
  !
